...
class Gui:
    ...
    def show_popup_menu(self, event):
        ...
        menu.add_command(label="Rotar") #command=self.rotar)
    ...
    def mover(self):
        if self.current_object:
            self.current_object.delete()
            self.current_object.moverX_Y(*self.puntos[0],*self.puntos[1],self.color.get())
    ...
    class Line():
        ...
        def moverX_Y(self, from_x, from_y, to_x, to_y, color='black'):
            updated_points = []
            for x, y in self.puntos:
                delta_x = x - from_x ; delta_y = y - from_y
                updated_points.append((to_x + delta_x, to_y + delta_y))
            self.puntos = updated_points
            self.delete()
            self.draw(*self.puntos[0], *self.puntos[1], color=color, grosor=self.grosor)
    ...
    class Circle():
        ...
        def moverX_Y(self, from_x, from_y, to_x, to_y, color='black'):
            x_c,y_c = self.puntos[0]
            x_r,y_r = self.puntos[1]
            delta_x_c = x_c - from_x ; delta_y_c = y_c - from_y 
            delta_x_r = x_r - from_x ; delta_y_r = y_r - from_y  
            self.puntos[0] = (to_x + delta_x_c, to_y + delta_y_c)
            self.puntos[1] = (to_x + delta_x_r, to_y + delta_y_r)
            self.draw(*self.puntos[0], *self.puntos[1], color=color, grosor=self.grosor)
    ...
    class Triangulo(Line):
        ...
        def moverX_Y(self, from_x, from_y, to_x, to_y,color='black'):
            ...
            for x, y in self.puntos:
                ...
                updated_points.append((to_x + delta_x, to_y + delta_y))
            self.puntos = updated_points
            self.draw(*self.puntos[0], *self.puntos[1], *self.puntos[2], color=color, tipo="Normal", grosor=self.grosor)
